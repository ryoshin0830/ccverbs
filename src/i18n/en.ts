...
export const en = {
  ...
  help: {
    ...
    agentsHeading: "For AI agents:",
    agents: [
      "Every command takes --json and prints one JSON object whose first key is ok.",
      "set, random, and reset only print the diff until you add --yes.",
      "The bare picker needs a TTY, so drive ccverbs with these instead:",
    ],
    agentExamples: [
      { cmd: "ccverbs list --json", text: "Read the catalog before choosing" },
      { cmd: "ccverbs show <id> --json", text: "Read every verb in one set" },
      { cmd: "ccverbs search <query> --json", text: "Find sets by id, name, tag" },
      { cmd: "ccverbs set <id> --json", text: "Preview the settings diff only" },
      { cmd: "ccverbs set <id> --yes --json", text: "Apply it for the user" },
      { cmd: "ccverbs current --json", text: "Report what is applied now" },
    ],
    contributeHeading: "Adding a verb set (for AI agents):",
    contribute: [
      "1. ccverbs list --json — check that the theme is not covered yet.",
      "2. Draft one JSON object with id, name, emoji, description, language,",
      "   category, tags, and 10-40 verbs in a single voice.",
      "3. cat set.json | ccverbs new --input - --json validates offline;",
      "   repair every error.issues entry by its path and code, then repeat.",
      "4. Show the canonical JSON to the user. Add --pr only after they",
      "   explicitly authorize opening a pull request.",
      "Full contract: ccverbs new --help and docs/ai-agents.md",
      "A human can instead run ccverbs and choose \"Create a new set\".",
    ],
    new: {
