...
メタ情報（`author` `source` `tags` と実際の長さの `description`）を含む
現実的なセットでの実測:

| 想定 | URL | 判定 |
| --- | --- | --- |
| 学習系 24 語（同梱セットの実際の語数） | 3,993 | inline |
| 学習系 40 語 | 6,009 | inline |
| **学習系 40 語（40 列に近い長さ）** | **6,929** | inline |
| 日本語 40 語 | 5,609 | inline |
| 学習系 60 語（40 列に近い長さ） | 9,909 | フォールバック |
| 長い日本語 100 語 | 17,069 | フォールバック |

**上限は 7,500 文字とする。** 広く実装されている HTTP リクエストライン
上限 8 KB に対する保守的な値である。GitHub の実際の 414 閾値を計測した値では
ないため、余裕を取る。

この値の根拠として重要なのは、**メタ情報を含む学習系 40 語が 6,009〜6,929 文字に
なる**という実測である。学習系はこのプロジェクトで最も価値の高いカテゴリであり、
ここが常にフォールバックに落ちる設計は失敗である。当初 6,000 と見積もっていたが、
実測により 7,500 へ引き上げた。60 語級から先はフォールバックに入る。
...
| `newFileUrl` | 日本語 40 語（実測 5,609）が `tooLong: false` であること |
| `newFileUrl` | **メタ情報込みの学習系 40 語（実測 6,009）が `tooLong: false` であること** — 当初の 6,000 上限で落ちた回帰を防ぐ |
| `newFileUrl` | **40 列に近い学習系 40 語（実測 6,929）が `tooLong: false` であること** |
| `newFileUrl` | 学習系 60 語（実測 9,909）と長い 100 語（実測 17,069）で `tooLong: true`、`url` が null、`fallbackUrl` は常に返ること |
