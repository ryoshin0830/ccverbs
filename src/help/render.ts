...
export function renderHelp(t: Catalog, command?: HelpCommand): string {
  ...
  return [
    ...
    "",
    t.help.agentsHeading,
    ...t.help.agents.map((line) => `${INDENT}${line}`),
    "",
    ...table(t.help.agentExamples.map((e) => ({ left: e.cmd, right: e.text }))),
    "",
    t.help.contributeHeading,
    ...t.help.contribute.map((line) => `${INDENT}${line}`),
    "",
    t.help.exitCodes,
    "",
    t.help.footer,
