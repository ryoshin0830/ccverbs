...
検索できるピッカーが開きます。セットを選び、差分を確認して、適用 — 聞かれるのは2回だけです。`settings.json` の他のキーには一切触りません。一覧には **新しい単語セットを作る** もあり、選ぶと投稿用Webアプリが開きます。
...
`npx ccverbs` の最初の一覧で **新しい単語セットを作る** を選んでも同じ画面を開けます。
端末からブラウザを起動できない場合はURLが表示されるので、手動で開いてください。
...
`ccverbs new --help` はエージェント専用の詳細ガイドです。入力形式、最小JSON例、
検証と修正のループ、構造化された `error.issues`、`--pr` の明示的な許可条件を表示します。
...
この取り決めは CLI 自身が持っています。`npx ccverbs --help` の末尾に **AI エージェント向け** と **単語セットを追加する** の節があり、`npx ccverbs new --help` がコマンド単位の詳細ページです。`--help` を読むだけで手順が一通り分かります。
